...
async function fetchDebate(debateId: string) {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/v1/debates/public/${debateId}`, {
      next: { revalidate: 300 },
    });
    if (res.ok) {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/v1/playground/debate/${debateId}`, {
      next: { revalidate: 300 },
    });
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!debate) {
    return new ImageResponse(
      ...
      >
        <div
          style={{ fontSize: 28, fontFamily: 'monospace', color: '#39ff14', letterSpacing: '4px' }}
        >
          ARAGORA
        </div>
        <div style={{ fontSize: 18, fontFamily: 'monospace', color: '#9a9a9a', marginTop: '16px' }}>
  ...
  const verdict =
    debate.verdict?.length > 160 ? debate.verdict.slice(0, 157) + '...' : (debate.verdict ?? '');
    ...
    >
      ...
      <div
        style={{
          ...
          right: 0,
          height: '3px',
          background: 'linear-gradient(90deg, #39ff14, #00ffff, #bf00ff, #ffd700, #ff0040)',
        }}
      />
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div style={{ fontSize: 12, fontFamily: 'monospace', color: '#9a9a9a' }}>
                CONFIDENCE
              </div>
              {/* Confidence bar */}
              <div
              ...
              <div
                style={{ fontSize: 14, fontFamily: 'monospace', color: '#39ff14', fontWeight: 700 }}
              >
                {confidencePercent}%
              </div>
            </div>
          </div>
          <div style={{ fontSize: 16, fontFamily: 'monospace', color: '#e0e0e0', lineHeight: 1.4 }}>
            {verdict}
          </div>
        </div>
      )}
