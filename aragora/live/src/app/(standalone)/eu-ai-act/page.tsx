...
type ClassificationResponse = RiskClassification | { classification?: RiskClassification | null };
...
const SAMPLE_USE_CASES = [
  {
    label: 'Hiring AI',
    description:
      'AI system that screens job applicants, scores resumes, and recommends candidates for interview based on historical hiring data.',
  },
  {
    label: 'Medical Diagnosis',
    description:
      'AI-assisted diagnostic system that analyzes medical imaging and patient history to suggest differential diagnoses.',
  },
  {
    label: 'Credit Scoring',
    description:
      'Automated creditworthiness scoring system that evaluates loan applications using financial history and behavioral patterns.',
  },
  {
    label: 'Content Recs',
    description:
      'AI system that recommends articles and videos based on browsing history for a news platform.',
  },
];
...
const ARTICLES_PREVIEW = [
  {
    ...
    icon: '\uD83D\uDCDD',
    description:
      'Complete event logs of every debate phase — propose, critique, revise, vote, synthesize. Technical documentation per Annex IV. Configurable retention policies.',
    fields: [
      'Event log (47+ event types)',
  ...
  },
  {
    ...
    icon: '\uD83D\uDD0D',
    description:
      "Provider identity, intended purpose, known risks, and output interpretation guidance. Every agent's model identity disclosed in decision receipts.",
    fields: [
  ...
  },
  {
    ...
    icon: '\uD83D\uDC64',
    description:
      'Human-on-the-loop approval via receipt gating. Override and stop mechanisms. Automation bias safeguards via contrarian agents and dissent tracking.',
    fields: [
      'Oversight model definition',
      'Bias safeguards (3+ mechanisms)',
      'Override & kill switch docs',
    ],
  },
];
...
function getDemoClassification(description: string): RiskClassification {
  ...
  const isUnacceptable =
    lower.includes('social scoring') ||
    lower.includes('subliminal') ||
    lower.includes('biometric categorization');

  if (isUnacceptable) {
    return {
      risk_level: 'unacceptable',
      annex_iii_categories: ['Article 5 prohibited practices'],
      applicable_articles: ['Article 5'],
      matched_keywords: ['social scoring'],
      confidence: 0.95,
    };
  }
  if (isHigh) {
    const categories = lower.includes('hiring')
      ? ['Employment, workers management']
      : lower.includes('credit') || lower.includes('loan')
        ? ['Access to essential private and public services']
        : ['Medical devices (Annex II)'];
    const keywords = lower.includes('hiring')
      ? ['hiring', 'applicant']
      : lower.includes('credit')
        ? ['credit', 'scoring']
        : ['diagnostic', 'medical'];
    return {
      risk_level: 'high',
      annex_iii_categories: categories,
...
function normalizeClassificationResponse(data: ClassificationResponse): RiskClassification | null {
...
export default function CompliancePage() {
  ...
  const classify = useCallback(async () => {
    ...
    try {
      const data = await apiPost<ClassificationResponse>('/api/v2/compliance/eu-ai-act/classify', {
        description: useCase,
      });
  ...
  const riskLabel = classification
    ? RISK_STYLES[classification.risk_level]?.label || 'CLASSIFIED'
    : '';
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      ...
      <section
        ...
        style={{ paddingTop: '96px', paddingBottom: '80px', fontFamily: 'var(--font-landing)' }}
      >
        <div className="max-w-2xl mx-auto">
          ...
          >
            EU AI Act Compliance <span style={{ color: 'var(--accent)' }}>in Minutes</span>
          </h1>
          ...
          >
            Classify your AI system&apos;s risk level, then generate Article 12, 13, and 14
            compliance artifacts from any Aragora decision receipt.
          </p>
      ...
      >
        <div className="max-w-2xl mx-auto">
          ...
          <p
            ...
            style={{ fontSize: '15px', color: 'var(--text-muted)', marginBottom: '40px' }}
          >
      ...
      >
        <div className="max-w-3xl mx-auto">
          ...
          <p
            className="text-center max-w-md mx-auto"
            style={{ fontSize: '15px', color: 'var(--text-muted)', marginBottom: '64px' }}
          >
            Every Aragora decision receipt generates a complete EU AI Act compliance bundle covering
            the three mandatory articles for high-risk systems.
          </p>
      ...
      >
        <div className="max-w-lg mx-auto">
          ...
          >
            Run a multi-model debate on any decision. Aragora generates a cryptographically signed
            compliance bundle from the result.
          </p>
          <Link
            href="/playground"
