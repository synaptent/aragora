      ...
      actionsContent: (
        <div className="space-y-2">
          <button
            ...
            className="block w-full px-3 py-2 text-xs font-theme-data text-center bg-[var(--acid-green)] text-[var(--bg)] font-bold hover:bg-[var(--acid-green)]/80 transition-colors"
          >
            PUBLISH TEMPLATE
          </button>
          <Link
    ...
    <>
      ...
      <main className="min-h-screen bg-bg text-text relative z-10">
        <div className="max-w-6xl mx-auto px-4 py-8">
          {/* Header */}
          <div className="mb-6">
            <h1 className="text-2xl font-theme-data text-[var(--accent)] mb-2">MARKETPLACE</h1>
          ...
          {/* Tab Switcher */}
          <div className="mb-6 flex gap-0 border-b border-[var(--accent)]/20">
            <button
              onClick={() => setActiveTab('catalog')}
              className={`px-6 py-2 text-sm font-theme-data transition-colors border-b-2 ${
                activeTab === 'catalog'
                  ? 'text-[var(--accent)] border-[var(--accent)]'
                  : 'text-text-muted border-transparent hover:text-text hover:border-[var(--accent)]/30'
              }`}
            >
              CATALOG
            </button>
              ...
              <div className="mb-6 space-y-4">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-4 py-2 bg-surface border border-[var(--accent)]/30 text-text font-theme-data text-sm focus:outline-none focus:border-[var(--accent)]"
                  />
                  <select
                    value={sortBy}
                    onChange={(e) => setSortBy(e.target.value as 'downloads' | 'stars' | 'recent')}
                    className="px-4 py-2 bg-surface border border-[var(--accent)]/30 text-text font-theme-data text-sm focus:outline-none"
                  >
              ...
              {isLoading ? (
                <div className="flex items-center justify-center py-16">
                  <div className="text-center">
                    ...
                    <p className="text-text-muted text-sm font-theme-data">Loading listings...</p>
                  </div>
                </div>
              ) : filteredListings.length === 0 ? (
                <div className="border border-[var(--accent)]/20 bg-surface/30 p-8 text-center">
                  <p className="text-text-muted text-sm font-theme-data mb-2">No listings found</p>
            ...
            <>
                    ...
                    <button
                      ...
                      className={`px-3 py-1 text-xs font-theme-data border transition-colors ${
                        selectedType === type.value
                          ...
                          : 'bg-surface text-text-muted border-border hover:border-[var(--acid-cyan)]/30'
                      }`}
                    >
                      {type.label}
                    </button>
              ...
              {isLoading ? (
                <div className="flex items-center justify-center py-16">
                  <div className="text-center">
                    <div className="w-8 h-8 border-2 border-[var(--accent)]/30 border-t-acid-green rounded-full animate-spin mx-auto mb-4" />
                    <p className="text-text-muted text-sm font-theme-data">Loading templates...</p>
                  </div>
                </div>
              ) : filteredTemplates.length === 0 ? (
                <div className="border border-[var(--accent)]/20 bg-surface/30 p-8 text-center">
                  <p className="text-text-muted text-sm font-theme-data mb-2">No templates found</p>
                  <p className="text-text-muted/60 text-xs font-theme-data">
                    Try adjusting your filters or search query.
                  </p>
                </div>
  ...
  return (
    <div
      ...
      className={`border bg-surface/50 p-4 cursor-pointer transition-colors hover:bg-surface/80 ${
        featured ? 'border-[var(--accent)]/50' : 'border-[var(--accent)]/20'
      }`}
    >
      {featured && (
        <div className="text-xs font-theme-data text-[var(--accent)] mb-2 uppercase">Featured</div>
      )}

      {/* Header */}
      <div className="flex items-start justify-between mb-2">
        <div>
          <h3 className="text-sm font-theme-data text-text font-bold">{listing.name}</h3>
      ...
      <div className="flex flex-wrap gap-1 mb-3">
        {listing.tags.slice(0, 3).map((tag) => (
          <span
            key={tag}
            className="px-2 py-0.5 text-xs font-theme-data bg-bg/50 text-text-muted border border-border"
          >
            {tag}
          </span>
        ))}
      ...
      <div className="flex items-center justify-between text-xs font-theme-data">
        <div className="flex gap-4">
          <span className="text-text-muted">
            <span className="text-[var(--accent)]">{listing.downloads}</span> installs
          </span>
          <span className="text-text-muted">
        ...
        <div className="flex gap-1">
          {onLaunchDebate && (listing.type === 'template' || listing.type === 'agent_pack') && (
            <button
              ...
              className="px-2 py-1 text-xs font-theme-data bg-[var(--acid-cyan)]/10 text-[var(--acid-cyan)] border border-[var(--acid-cyan)]/30 hover:bg-[var(--acid-cyan)]/20 transition-colors"
            >
              LAUNCH DEBATE
            </button>
          )}
          <button
            ...
            className="px-2 py-1 text-xs font-theme-data bg-yellow-500/10 text-yellow-400 border border-yellow-500/30 hover:bg-yellow-500/20 transition-colors"
          >
            RATE
          </button>
          <button
            ...
            className={`px-2 py-1 text-xs font-theme-data transition-colors ${
              isInstalled
...
function ListingDetailModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-bg/80 backdrop-blur-sm">
      <div className="w-full max-w-2xl max-h-[90vh] overflow-auto bg-surface border border-[var(--accent)]/30 m-4">
        {/* Header */}
        ...
        <div className="p-4 space-y-4">
          ...
          {/* Tags */}
          <div>
            <h3 className="text-xs font-theme-data text-[var(--acid-cyan)] uppercase mb-2">Tags</h3>
        ...
        {/* Actions */}
        <div className="sticky bottom-0 bg-surface border-t border-[var(--accent)]/20 p-4 flex gap-2">
          <button
            onClick={onInstall}
            disabled={isInstalled}
            className={`flex-1 py-2 font-theme-data font-bold transition-colors ${
              isInstalled
                ? 'bg-surface text-text-muted border border-border cursor-not-allowed'
          ...
          <button
            onClick={onRate}
            className="px-4 py-2 font-theme-data bg-yellow-500/10 text-yellow-400 border border-yellow-500/30 hover:bg-yellow-500/20 transition-colors"
          >
            RATE
          </button>
        </div>
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-bg/80 backdrop-blur-sm">
      <div className="w-full max-w-md bg-surface border border-[var(--accent)]/30 m-4 p-6">
        ...
        <p className="text-xs font-theme-data text-text-muted mb-6">
          {LISTING_TYPE_LABELS[listing.type]} by {listing.author} &middot; v{listing.version}
        </p>
        <div className="flex gap-2">
          <button
            onClick={onConfirm}
            className="flex-1 py-2 bg-[var(--accent)] text-bg font-theme-data font-bold hover:bg-[var(--accent)]/80 transition-colors"
          >
            INSTALL
          </button>
          <button
            onClick={onCancel}
            className="flex-1 py-2 bg-surface text-text-muted border border-border font-theme-data hover:border-[var(--accent)]/30 transition-colors"
          >
            CANCEL
          </button>
        </div>
          ...
          <div className="flex gap-2">
            {[1, 2, 3, 4, 5].map((s) => (
              <button
                key={s}
                onClick={() => setScore(s)}
                className={`w-10 h-10 text-sm font-theme-data border transition-colors ${
                  score >= s
...
function TemplateCard({
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-2">
        <div>
          <h3 className="text-sm font-theme-data text-text font-bold">{metadata.name}</h3>
      ...
      <div className="flex items-center justify-between text-xs font-theme-data">
        <div className="flex gap-4">
          <span className="text-text-muted">
            <span className="text-[var(--accent)]">{metadata.downloads}</span> downloads
          </span>
          <button
            onClick={(e) => {
              e.stopPropagation();
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex gap-6">
            ...
            <div>
              ...
              <div className="text-lg font-theme-data text-yellow-400">{metadata.stars}</div>
            </div>
            <div>
              <span className="text-xs font-theme-data text-text-muted">License</span>
              <div className="text-sm font-theme-data text-text">{metadata.license}</div>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="space-y-1">
                {template.nodes.slice(0, 5).map((node, i) => (
                  <div key={i} className="text-xs font-theme-data text-text-muted">
                    {node.id}: {node.type}
                  </div>
                ))}
                {template.nodes.length > 5 && (
                  <div className="text-xs font-theme-data text-text-muted/60">
      ...
      <div className="w-full max-w-lg bg-surface border border-[var(--accent)]/30 m-4">
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <button
            ...
            className="w-full py-2 bg-[var(--accent)] text-bg font-theme-data font-bold hover:bg-[var(--accent)]/80 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
