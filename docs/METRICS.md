...
| Test files (test_*.py under tests/) | `5643` | `tests/` | `git ls-files tests \| grep -E '(^\|/)test_[^/]*\.py$' \| wc -l` |
| Test functions (class + module level) | `227404` | `tests/` | `git grep -E '^[[:space:]]*(async )?def test_' -- tests \| wc -l` |
| @pytest.mark.parametrize decorators | `1462` | `tests/` | `git grep -E '@pytest\.mark\.parametrize' -- tests \| wc -l` |
