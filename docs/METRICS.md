...
|---|---|---|---|
| Python files under aragora/ | `4333` | `aragora/` | `git ls-files aragora \| grep -E '\.py$' \| wc -l` |
| Python lines of code under aragora/ | `2006073` | `aragora/` | `python3 -c "from pathlib import Path; import subprocess; files = subprocess.check_output(['git', 'ls-files', 'aragora'], text=True).splitlines(); print(sum(sum(1 for _ in Path(p).open(encoding='utf-8', errors='replace')) for p in files if p.endswith('.py')))"` |
...
| Test functions (class + module level) | `227095` | `tests/` | `git grep -E '^[[:space:]]*(async )?def test_' -- tests \| wc -l` |
| @pytest.mark.parametrize decorators | `1335` | `tests/` | `git grep -E '@pytest\.mark\.parametrize' -- tests \| wc -l` |
