...
| CHR-X-014 | REMOVED | `aragora/tasks/`; task-routing authority moved into `aragora/server/handlers/tasks/execution.py`, with dedicated tests at `tests/server/handlers/tasks/test_task_router.py` | one implementation module at the live handler boundary | 2026-07-20 | #8851 operator ruling; #9057 |
...
| CHR-E-004 | Server-local metrics/tracing/http-pool homes at `aragora/server/metrics.py`, `aragora/server/middleware/tracing.py`, and `aragora/server/http_client_pool.py` | re-adding those retired homes is excluded; unrelated handler code under `aragora/server/` remains outside this path rule | P4a; see CHR-P4A-001..003 |
