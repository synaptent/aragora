...
| Variable | Required | Description | Default |
|----------|----------|-------------|---------|
| `ARAGORA_ODR_PROFILE_VERSION` | Optional | ODR export profile (`0.1` or `0.2`) for `aragora receipt export --format odr` and `scripts/emit_pr_receipt.py`. `--odr-version` overrides it; empty means unset; invalid values fail with usage exit 2 before writing. The default is 0.2 from release 2.11.0; set `0.1` to keep emitting v0.1. | `0.2` |
...
| `ARAGORA_ODR_SIGNING_ISSUER` | Optional | `signatures[].issuer` written by `aragora receipt export --format odr` and `scripts/emit_pr_receipt.py` on v0.2 documents, the default profile (signer-committed, spec §6); empty means unset. Ignored for v0.1 documents, which keep the three-member entry. | `aragora` |
