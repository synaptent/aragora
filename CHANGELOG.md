...
- **`CLIAgent.SENDS_MODEL_ON_WIRE`:** says whether a CLI agent really puts `self.model` on its command line. It defaults to **False** and is set True only on the six CLIs that pass a model flag (claude, codex, gemini-cli, grok-cli, antigravity, openai), so an agent nobody classified cannot have its answer attributed to a model the CLI never received; the runner records such an agent's served model as `unknown (CLI default)`. `ARAGORA_ANTIGRAVITY_EFFORT` is now validated against `low|medium|high` (an unrecognised value warns and uses `medium` rather than erroring every `agy` call).
- **`DecisionReceipt.served_models`:** receipts now name the model that ACTUALLY answered whenever a provider served a different one than the debate pinned (Anthropic's server-side refusal fallback), or the requested id never reached the wire at all (an unpinned CLI agent, recorded as `unknown (CLI default)`). The claim is debate-wide — computed from each agent's per-call log, cleared at debate start — and carries `calls`/`fallback_calls` alongside the distinct served ids. The field binds into `artifact_hash` whenever it is non-empty, so the statement cannot be edited after signing; a receipt without it hashes exactly as before and keeps verifying. The call's token usage is priced against the model that answered (`APIAgent.billing_model`) in the monthly budget guard and both per-round cost trackers, so a Fable request served by Opus 4.8 is no longer costed at Fable's 2x rate and a receipt's cost summary agrees with its `served_models` block.
...
- **Default output-token budgets raised, and OpenAI reasoning rows now send `reasoning_effort`.** Two per-call spend changes ship together and are called out here because both move real money. (1) The Anthropic agent's default `max_tokens` moves from a flat 4096 to `min(catalog max_output_tokens, 16_000)` non-streaming / `min(..., 64_000)` streaming, and the OpenAI-compatible builder does the same (`min(catalog max_output_tokens, 16_000)`) for any model it sends `reasoning_effort` to, because on OpenAI reasoning models the reasoning tokens are billed and capped *inside* `max_completion_tokens` and a 4096 budget at high effort returns empty completions. (2) The builder now also sends the catalog's `reasoning_effort_default` for reasoning-class rows — `"high"` for the new default `gpt-6-astra`, `"medium"` for the value-tier `gpt-5.6-terra` — where nothing was sent before. **Together these raise worst-case per-call output spend by up to 4x (Anthropic non-streaming) to 16x (Anthropic streaming), on top of whatever additional billed reasoning tokens high effort produces, and the fail-closed monthly budget guard is checked before the call rather than during it — so operators should expect the per-call cost and latency profile of default-configured agents to move and should re-check `ARAGORA_MONTHLY_BUDGET_USD`.** An explicit caller value is still respected (capped at the catalog maximum on the Anthropic path), an explicit per-instance `reasoning_effort` still wins, and a model with no catalog row, or any non-reasoning model, keeps the old 4096 and sends no effort field. The streamed Anthropic default is settable: `ARAGORA_ANTHROPIC_STREAM_MAX_TOKENS` (`agent.anthropic_stream_max_tokens`, default 64,000) lowers it without touching the catalog, which matters because `generate_stream` exposes no caller `max_tokens` and the default is therefore that path's only output ceiling; the catalog's `max_output_tokens` still caps the result, and an explicit caller value still wins.
- **`XAI_BASE_URL` and `MISTRAL_BASE_URL` are now honoured** by the Grok and Mistral agents, matching the existing `ANTHROPIC_BASE_URL` / `OPENAI_BASE_URL` BYOK-gateway support (issue #9304). Neither variable was read anywhere before this release, and neither is set in this repo, its workflows or its deploy files, so nothing changes for an environment that does not set them; where one is set, that agent now targets the gateway and skips the constructor-time retired-model-id rewrite.
...
- **`YiAgent` and `Qwen35PlusAgent`, and the `yi` / `qwen-3.5` agent types.** Both models (01.AI Yi Large / `01-ai/yi-large`, Qwen3.5 Plus / `qwen/qwen3.5-plus-02-15`) are gone from the live OpenRouter catalog, so the registrations could no longer build. Both classes are dropped from `aragora.agents.api_agents.__all__`, and the agent types are pruned from `config/provider_readiness.py`, `agents/credential_validator.py`, `config/settings.py`, `agents/base.py`, `config/legacy.py`, `agents/personas/defaults.py` and `agents/transports/burnin.py` so `aragora doctor` / `aragora status` no longer advertise them and a config naming them fails validation instead of failing later at `AgentRegistry` construction. Existing spellings still price and resolve: `qwen/qwen3.5-plus-02-15` upgrades to `qwen3.8-2.4t-a95b` and `01-ai/yi-large` retains its OpenRouter fallback entry.
