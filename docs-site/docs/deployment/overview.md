...
**Available tags:** `latest` (main branch HEAD), `2.11.0` (version from pyproject.toml), `v2.11.0` (git tag), `<major>.<minor>`, `<sha>`.
...
- Production hardening (TLS, secrets, resource limits): [PRODUCTION_DEPLOYMENT.md](./production-deployment).
- Persistent volumes and bind mounts: [CONTAINER_VOLUMES.md](./container-volumes).
- Database provisioning: [DATABASE_SETUP.md](./database-setup).
...
services:
  aragora-backend:
    image: ghcr.io/synaptent/aragora/backend:2.11.0  # Pin to known-good version
```

Or via environment variable:

```bash
ARAGORA_BACKEND_IMAGE=ghcr.io/synaptent/aragora/backend:2.11.0 docker compose up -d
```
